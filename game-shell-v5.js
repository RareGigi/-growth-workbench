...
(function(){
  ...
  function isolatedPiece(slot){
    if(!ready) return `<svg viewBox="0 0 1254 1254" aria-hidden="true"></svg>`;
    if(slot==='outer') return `<svg viewBox="0 0 1254 1254" role="img" aria-label="${META[slot].name}">${img('outer_back',POS.outer_back)}<image href="${FRONT}?v=${VERSION}" x="0" y="0" width="1254" height="1254"/></svg>`;
    return `<svg viewBox="0 0 1254 1254" role="img" aria-label="${META[slot].name}">${img(slot,POS[slot])}</svg>`;
  }
  function piecePreview(slot){return isolatedPiece(slot)}
  ...
  function loadB64(k,path){return fetch(`${path}?v=${VERSION}`,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(path);return r.text()}).then(t=>new Promise((res,rej)=>{const u='data:image/webp;base64,'+t.replace(/\s+/g,'');const im=new Image();im.onload=()=>{src[k]=u;res()};im.onerror=rej;im.src=u}))}
