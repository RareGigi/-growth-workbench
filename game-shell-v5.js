(function(){
  ...
  const OUTER_CHUNKS=[0,1,2,3,4].map(i=>`assets/dressup-v30/outer-0${i}.b64`);
  const LAYERS={
  ...
  let scene='wardrobe',tab='套装',draft={...state.mistV31.equipped},src={},ready=false,failed=false;

  const old=document.getElementById('gameAppV5');if(old)old.remove();
  const app=document.createElement('section');app.id='gameAppV5';app.className='gameAppV5 dressupApp dressupV31';
  ...
  function figure(l=draft,mini=false){if(complete(l))return `<div class="v23Figure ${mini?'mini':''} outfitFigure"><img src="${OUTFIT}?v=${VERSION}" alt="雾城漫步·银灰旅装" draggable="false"></div>`;if(!ready)return `<div class="v23Figure ${mini?'mini':''} outfitFigure"><img src="${OUTFIT}?v=${VERSION}" alt="换装资源载入中" draggable="false"><span class="v31Loading">${failed?'透明图层载入失败':'正在准备高清透明图层…'}</span></div>`;return `<div class="v31Figure ${mini?'mini':''}">${partialSvg(l)}</div>`}
  ...
  function drawer(){if(tab==='套装')return `<article class="v23SetCard ${complete(draft)?'selected':''}"><div class="v23SetPreview">${figure(FULL,true)}</div><div class="v23SetInfo"><small>SSR · V31B FIXED LAYERS</small><h3>雾城漫步·银灰旅装</h3><p>完整套装保持高清原图；拆分使用 v30 高清人物与透明外套。</p><button data-wear-set>整套试穿</button></div></article>`;const slot=Object.keys(META).find(s=>META[s].tab===tab);return `<div class="v31PieceGrid">${pieceCard(slot)}</div>`}
  ...
  function verify(k,b64){return new Promise((res,rej)=>{const u='data:image/webp;base64,'+b64;const im=new Image();im.onload=()=>{src[k]=u;res()};im.onerror=()=>rej(new Error('decode '+k));im.src=u})}
  Promise.all([
    Promise.all(BASE_CHUNKS.map(fetchText)).then(parts=>verify('base',parts.join(''))),
    Promise.all(OUTER_CHUNKS.map(fetchText)).then(parts=>verify('outer',parts.join(''))),
    ...Object.entries(LAYERS).map(([k,p])=>fetchText(p).then(t=>verify(k,t)))
  ]).then(()=>{ready=true;failed=false;if(app.classList.contains('open'))render()}).catch(err=>{console.error('v31b layer load failed',err);failed=true;if(app.classList.contains('open'))render()});
})();
