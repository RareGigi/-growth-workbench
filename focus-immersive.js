(() => {
  ...
  const MUSIC = [
    ...
    ['constellations', '温柔星群'],
    ['temple-dawn', '晨钟之前'],
    ['rain-brushstrokes', '雨中笔触'],
    ['breezy-terrace', '风过露台'],
    ['lamplight-chapter', '灯下章节'],
    ['glasshouse-ghosts', '雾中花房'],
    ['snow-needle', '雪落唱针'],
    ['sea-glass', '海玻璃暮光'],
    ['last-train', '末班车灯']
  ];
  const PRESETS = Core.FOCUS_SOUND_PRESETS;
  ...
  function applyRecommended() {
    ...
    settings.soundscapePresetVersion = 2;
    try { Core.save('focus-recommended-soundscape'); } catch (_) {}
    forwardTrack('ambience', preset.ambienceId);
    scheduleEnhance();
  }
