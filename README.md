...
一个无需账号与后端的静态个人成长 App。它把今日任务、收集箱、项目、日历、专注、习惯、笔记、周复盘与月度纪念串成真实数据闭环，再用星星、金币、贴纸、徽章和限定卡面提供轻量收藏反馈。

界面使用独立纸张、装订、信笺与植物标本拼贴，以及 25 枚模切 SVG 贴纸素材形成克制的电子手账语言；其中 8 枚新增贴纸已补充枝叶、邮票、纸张、纹理与陪衬小物，不再只是简易图标。贴纸只在真实行动解锁后进入固定收藏位。收藏系统包含 16 套角色卡面与 12 枚成长徽章；16 间自习室则只保留在专注页，避免和收藏重复。

今日支持一句话添加，也能按可用时间、状态与税务师 / 英语 / 播客 / CPA / 写作五个方向生成三套可直接采用的安排。“智能拆任务”可从任意一句任务推导目标、资料、步骤、风险、验收与第一步；默认在本地运行，只有用户明确点击时才会把当前任务交给已配置的 AI 服务。初次打开不会填入虚构完成数据；所有统计都来自用户真实记录。
...
focus-immersive.js       # 16 间自习室、动态、筛选与声音面板
...
持久化范围包括任务、项目、收集箱、智能拆解流程、专注会话与运行中计时器、自习室声音偏好、房间选择、习惯、心情、日记、成长记录、轨迹、笔记、周复盘、规划偏好与采用记录、金币、成长值、卡面、贴纸、徽章、收藏与月度纪念。

AI 深度拆解通过部署环境可选的 `window.GROWTH_AI_ENDPOINT` 接入安全服务端点。静态前端不存储 API 密钥，也不会自动发送任务、日记或整份状态；未配置端点时会明确保留本地方案。

自习室使用正式 `focusSettings.roomId` 保存当前房间；刷新、重新打开和 JSON 备份恢复后仍能回到上次选择。声音也写入同一份 `focusSettings`，没有第二套本地数据库。
...
房间面板支持筛选和“随便选”，列表在 iPhone 上独立纵向滚动。每个房间都有与画面对应的轻量动态；当前房间主图优先加载，手机端关闭高成本模糊和不必要的卡片动画，并忽略计时器高频文本更新，避免 Safari 重复重建图像面板。
